	...
	= fun l ->
		let l2 = find_fst l [] in
		let count = selflove_in l l2 0 in
		count
	...
	and selflove_in =
		fun l l2 count->
		match l2 with
		|hd :: tl -> let count' = count + likes l hd in selflove_in l tl count'
		|[] -> count

	and find_fst l l2 = 
		match l with
		|hd :: tl -> if (isMember l2 (fst hd)) then let l2' = l2@[fst hd] in find_fst tl l2' else find_fst tl l2
		|[] -> l2
	...
	and likes l person =
		...
		if(isMember l3 person) then 0 else 1
