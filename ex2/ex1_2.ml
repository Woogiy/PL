...
=	
	...
	if(calculate_in e1 env <= calculate_in e2 env) then
		calculate_in e3 env' +. calculate_in (SIGMA(ADD(e1,INT 1),(REAL(calculate_in e2 env)),e3)) env'
	else 0.0
	|INTEGRAL(e1,e2,e3) ->
	let env' = (X, calculate_in e1 env)::env in
	if((calculate_in (SUB(e2,e1)) env) > 0.1) then
		calculate_in (MUL(e3,REAL 0.1)) env' +. calculate_in (INTEGRAL(ADD(e1,REAL 0.1),(REAL(calculate_in e2 env)),e3)) env'
	else 0.0
